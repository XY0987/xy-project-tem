export const RNURL = 'direct:https://github.com/XY0987/myRnTem.git#main';
export const REACTURL = 'direct:https://github.com/XY0987/react-tem.git#main';
export const VUE2URL = 'direct:https://github.com/XY0987/Vue2_tem.git#main';
export const NUXTURL = 'direct:https://github.com/XY0987/nuxt_tem.git#main';
export const ELECTRONREACTURL =
  'direct:https://github.com/XY0987/electron-react-tem.git#main';
export const ELECTRONVUEURL =
  'direct:https://github.com/XY0987/electron-vue-tem.git#main';
